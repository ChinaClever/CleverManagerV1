...
BOOL CCanbinNPMDlg::OnEraseBkgnd(CDC* pDC)
{
	...
	if(master)
	{
		...
		name.Empty();
		name.Format("总功率:%.1fVA",master->Tcur[masterindex][0]*master->Tvol[masterindex][0]);
		...
		name.Empty();
		name.Format("门禁1:%s",master->Door1);
		BkDc.TextOut(100,660,name);

		name.Empty();
		name.Format("门禁2:%s",master->Door2);
	...
	if(salve)
	{
		...
		name.Empty();
		name.Format("总功率:%.1fVA",salve->Tcur[slaveindex][0]*salve->Tvol[slaveindex][0]);
		...
		name.Empty();
		name.Format("门禁1:%s",salve->Door1);
		BkDc.TextOut(520,660,name);

		name.Empty();
		name.Format("门禁2:%s",salve->Door2);
