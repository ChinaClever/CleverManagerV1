...
BOOL CCanbinNPMDlg::OnEraseBkgnd(CDC* pDC)
{
	...
	if(master)
	{
		...
		name.Empty();
		name.Format("水禁:%s",master->Water[masterindex]=="None."?"未接入":master->Water[masterindex]=="Alarm."?"报警":"正常");
		...
		name.Empty();
		name.Format("门禁1:%s",master->Door1[masterindex]=="None."?"未接入":master->Door1[masterindex]=="Opened."?"打开":"关闭");
		BkDc.TextOut(100,660,name);

		name.Empty();
		name.Format("门禁2:%s",master->Door2[masterindex]=="None."?"未接入":master->Door2[masterindex]=="Opened."?"打开":"关闭");
		BkDc.TextOut(100,680,name);

		name.Empty();
		name.Format("烟雾:%s",master->Smoke[masterindex]=="None."?"未接入":master->Smoke[masterindex]=="Alarm."?"报警":"正常");
	...
	if(salve)
	{
		...
		name.Empty();
		name.Format("水禁:%s",salve->Water[slaveindex]=="None."?"未接入":salve->Water[slaveindex]=="Alarm."?"报警":"正常");		
		...
		name.Empty();
		name.Format("门禁1:%s",salve->Door1[slaveindex]=="None."?"未接入":salve->Door1[slaveindex]=="Opened."?"打开":"关闭");		
		BkDc.TextOut(520,660,name);

		name.Empty();
		name.Format("门禁2:%s",salve->Door2[slaveindex]=="None."?"未接入":salve->Door2[slaveindex]=="Opened."?"打开":"关闭");
		BkDc.TextOut(520,680,name);

		name.Empty();
		name.Format("烟雾:%s",salve->Smoke[slaveindex]=="None."?"未接入":salve->Smoke[slaveindex]=="Alarm."?"报警":"正常");
