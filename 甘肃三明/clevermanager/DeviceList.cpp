...
BOOL CDeviceList::OnInitDialog()
{
	...
	NPMV = m_tree.InsertItem("NPDU",0,0,NULL);
...
void CDeviceList::drawListFrm(CDC *pDC)
{
	...
	pDC->TextOut(15,rect.Height()-149,"NPDU设备工作状态");
...
void CDeviceList::drawDevStaus(CDC *pDC)
{
	...
	drawFont(pDC,1,RGB(0,0,0));
	pDC->TextOut(rect.Width()*28/100+21,rect.Height()*60/100+11,"单台NPDU电力信息");
...
void CDeviceList::OnNMRclickTree1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		if(m_tree.GetItemText(hparent)=="NPMIII"||m_tree.GetItemText(hparent)=="NPMIV"||m_tree.GetItemText(hparent)=="NPDU"||m_tree.GetItemText(hparent)=="MS-PDU")
		{
			CMenu Popmenu;
			CMenu* pMenu;
			CPoint pt;
	...
	else if(hItem && !hparent&&hson)
	{
		...
		else if(m_tree.GetItemText(hItem)=="NPDU")
		{
			CMenu Popmenu;
			CMenu* pMenu;
			CPoint pt;
   ...
   for(i=0; i<10;i++)
   {
		...
		if(p_Objectnow->slaveOffline[i]==0) 
		{
			...
			{//Delay
			    ...
			    //if(0 == p_Objectnow->offlinerecord[i])//Normal
				{
					...
					p_Objectnow->Door1[i] = "None.";
					p_Objectnow->Door1[i] = "None.";
					p_Objectnow->Water[i] = "None.";
					p_Objectnow->Smoke[i] = "None.";
					CString str = "",name = "";
					CString content;
		...
		for(j=0;j<8;j++)
	   {
		    if((p_Objectnow->THmin[i][j]>p_Objectnow->TH[i][j] || p_Objectnow->THmax[i][j]<p_Objectnow->TH[i][j]) && p_Objectnow->THmax[i][j]>0)
		   {
				...
				p_Objectnow->THalarm[i][j] = 0;
			}
	   }

		if(p_Objectnow->Door1[i] == "Opened.")//门禁1不正常
		{ 
			if(p_Objectnow->Door1alarm[i] == 0) 
			{
		...
		{
			p_Objectnow->Door1alarm[i] = 0;
		}

	    if(p_Objectnow->Door2[i] == "Opened.")//门禁2不正常
		{
			if(p_Objectnow->Door2alarm[i] == 0) 
			{
		...
		{
			p_Objectnow->Door2alarm[i] = 0;
		}

		if(p_Objectnow->Smoke[i] == "Normal." || p_Objectnow->Smoke[i] == "None.")//正常
		{
			p_Objectnow->Smokealarm[i] = 0;
		}
		else//不正常
		{
			if(p_Objectnow->Smokealarm[i] == 0) 
			{
		...
		if(p_Objectnow->Water[i] == "Normal." || p_Objectnow->Water[i] == "None.")//正常
		{
			p_Objectnow->Wateralarm[i] = 0;
		}
		else//不正常
		{
			if(p_Objectnow->Wateralarm[i] == 0) 
			{
...
void CDeviceList::OnEditDevice()
{
	...
	else if(m_tree.GetItemText(parent)=="NPDU")
	{
		CMakeSureDlg sure;
		sure.pre_ip=pre_ip;
		sure.p_device = this;
