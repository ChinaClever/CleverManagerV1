...
IMPLEMENT_DYNAMIC(COutputLog, CDialog)

COutputLog::COutputLog(CWnd* pParent /*=NULL*/)
	: CDialog(COutputLog::IDD, pParent)
{
...
void COutputLog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(COutputLog, CDialog)
	ON_WM_ERASEBKGND()
	ON_WM_LBUTTONDOWN()
	ON_WM_NCHITTEST()
END_MESSAGE_MAP()
...
int COutputLog::zhang(CString str1 , CString str2 , CString str3)
{
...
BOOL COutputLog::OnEraseBkgnd(CDC* pDC)
{
...
void COutputLog::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if(point.x<285&&point.x>225&&point.y<190&&point.y>170)
	{
		...
		{ 
			...
			{ 
				...
				//str1.Empty();
				//int style;
				//style=((CComboBox*)GetDlgItem(IDC_COMBO1))->GetCurSel();
				//((CComboBox*)GetDlgItem(IDC_COMBO1))->GetLBText(style,str1);
				//AfxMessageBox(str1);
				/////////////////////////////////////////////////////////////////////查找日志
				...
				while(res)
				{
					...
					if(ff.IsDirectory() && !ff.IsDots())
					{
						...
						//int c1,c2,c3;
						//c1=atoi(strTitle.Left(4);
						//c2=atoi(strTitle.Mid(4,2);
						//c3=atoi(strTitle.Right(2);
						int aa=zhang(day1,day2,strTitle);
						if(aa!=0)
						{
							...
							//if(!makesureFile(getFilePath()+"\\logs\\"+strTitle+"\\users"))
							//{
							//return;
							//}
							file.Open(getFilePath()+"\\logs\\"+strTitle+"\\users",CFile::modeRead | CFile::modeCreate |CFile::modeNoTruncate);
							file.SeekToBegin();
							int i;
							i=0;
							while(1)
...
LRESULT COutputLog::OnNcHitTest(CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CRect rect;
	GetClientRect(&rect);
